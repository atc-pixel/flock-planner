...
export function ProductionWeeklyTable({ rows }: ProductionWeeklyTableProps) {
  ...
  const weeks = React.useMemo(() => {
    ...
    rows.forEach(row => {
        ...
        currentWeek.totalBroken += row.brokenEggCount; // YENİ
        currentWeek.totalDirty += row.dirtyEggCount;   // YENİ
  ...
  return (
    <div className="bg-white rounded-lg shadow overflow-hidden h-full flex flex-col">
        {/* Overflow-auto container sticky header için kritik */}
        <div className="overflow-auto grow"> 
            <table className="w-full text-xs text-left">
                <thead className="bg-slate-100 text-slate-500 font-bold border-b sticky top-0 z-20 shadow-sm">
                    <tr>
                        <th className="p-2 w-10 text-center bg-slate-100">Hafta</th>
                        <th className="p-2 bg-slate-100">Tarih Aralığı</th>
                        <th className="p-2 text-center bg-slate-100">Ölü</th>
                        <th className="p-2 text-center bg-slate-100">Yumurta</th>
                        <th className="p-2 text-center bg-slate-100 text-emerald-600">% Verim</th>
                        
                        {/* YENİ KOLONLAR */}
                        <th className="p-2 text-center bg-slate-100 text-red-500">Kırık %</th>
                        <th className="p-2 text-center bg-slate-100 text-amber-500">Kirli %</th>

                        <th className="p-2 text-center bg-amber-50 text-amber-800 border-x border-amber-100 w-24">
                            Yem (Kg)
                        </th>
                        <th className="p-2 w-32 bg-slate-100">Notlar</th>
                    </tr>
                </thead>
                    ...
                    {weeks.map((week) => {
                        ...
                        return (
                            ...
                            >
                                ...
                                <td className="p-2 text-center font-bold text-emerald-600">%{henDayYield.toFixed(2)}</td>
                                
                                {/* Kırık & Kirli Değerleri */}
                                <td className="p-2 text-center text-slate-500">%{brokenRate.toFixed(2)}</td>
                                <td className="p-2 text-center text-slate-500">%{dirtyRate.toFixed(2)}</td>
