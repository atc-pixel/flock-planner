...
export function ProductionWeeklyTable({ weeklyData }: ProductionWeeklyTableProps) {
  ...
  // NOT: Aggregation mantığı buradan kaldırıldı ve ProductionTable'a taşındı.
  ...
  // IŞINLANMA
  useLayoutEffect(() => {
    if (!loading && weeklyData.length > 0 && !scrollRef.current) {
        const today = new Date();
        const currentWeek = weeklyData.find(w => 
            isWithinInterval(today, { start: w.startDate, end: w.endDate })
        );

        const targetKey = currentWeek ? currentWeek.key : weeklyData[weeklyData.length - 1].key;
        ...
        <table className="w-full text-xs text-left">
            <thead className="bg-slate-50 text-slate-500 font-bold border-b sticky top-0 z-10">
                <tr>
                    ...
                    <th className="p-2 text-center text-slate-400 w-14">Kırık%</th>
                    <th className="p-2 text-center text-slate-400 w-14">Kirli%</th>
                ...
                {weeklyData.map((week) => {
                    ...
                    return (
                        ...
                        >
                            ...
                            <td className="p-2 text-center text-slate-500 text-[10px] font-mono">
                                {brokenRate > 0 ? `%${brokenRate.toFixed(2)}` : '-'}
                            </td>
                            <td className="p-2 text-center text-slate-500 text-[10px] font-mono">
                                {dirtyRate > 0 ? `%${dirtyRate.toFixed(2)}` : '-'}
                            </td>
